...
globalThis.addEventListener("load", () => {
    ...
    const loadUrl = new URLSearchParams(globalThis.location.search).get("load")
    if (loadUrl) {
        fetch(loadUrl)
    ...
    document.getElementById("load-img-input").addEventListener("input", (evt) => {
        ...
        for (const file of evt.target.files) {
            compress_image(file, (webpDataUrl) => {
                const img = create_img_with_src(webpDataUrl)
    ...
    document.getElementById("reset-list-input").addEventListener("click", () => {
        if (
            confirm("Reset Tierlist? (this will place all images back in the pool)")
        ) {
            soft_reset_list()
        }
    })
    ...
    document.getElementById("export-input").addEventListener("click", () => {
        const name = prompt("Please give a name to this tierlist")
        if (name) {
...
function compress_image(imageFile, callback) {
    ...
    reader.onload = (event) => {
        ...
        imgElement.onload = () => {
            ...
            const scale = MAX_IMG_HEIGHT / imgElement.height
...
function save_tierlist(filename) {
    const serialized_tierlist = {
        title: document.querySelector(".title-label").textContent,
        rows: [],
    }
    tierlist_div.querySelectorAll(".row").forEach((row, i) => {
        serialized_tierlist.rows.push({
            name: row.querySelector(".header label").textContent.substr(
                0,
                MAX_NAME_LEN,
            ),
        })
...
function load_tierlist(serialized_tierlist) {
    document.querySelector(".title-label").textContent = serialized_tierlist.title
    for (const idx in serialized_tierlist.rows) {
        ...
        elem.querySelector("label").textContent = ser_row.name
    }
    recompute_header_colors()
...
globalThis.addEventListener("dragend", end_drag)
...
function make_accept_drop(elem) {
    ...
    elem.addEventListener("drop", (evt) => {
        ...
        if (
            dragged_image_parent.tagName.toUpperCase() === "SPAN"
...
function enable_edit_on_click(container, input, label) {
    ...
    container.addEventListener("click", (_) => {
        ...
        input.value = label.textContent.substr(0, MAX_NAME_LEN)
...
function resize_headers() {
    ...
    for (const [, , label] of all_headers) {
        max_width = Math.max(max_width, label.clientWidth)
    }

    for (const [other_header, ..._] of all_headers) {
...
function add_row(index, name) {
    ...
    btn_rm.addEventListener("click", (evt) => {
        ...
        console.assert(idx >= 0)
        if (
            rows[idx].querySelectorAll("img").length === 0
            || confirm(
                `Remove tier ${
                    rows[idx].querySelector(".header label").textContent
                }? (This will move back all its content to the untiered pool)`,
            )
        ) {
            rm_row(idx)
        }
...
function bind_trash_events() {
    ...
    trash.addEventListener("dragover", evt => evt.preventDefault())
